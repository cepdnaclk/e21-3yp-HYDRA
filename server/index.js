...
let heavyVehicleTimeout = { North: null, South: null, East: null, West: null };
const HEAVY_VEHICLE_BONUS_DURATION = 30000; // 30 seconds bonus window
...
// ── Pedestrian Data ──────────────────────────────────────────────────────────
let pedStatus = {
    North: { requested: false, crossing: false, duration: 0, waitingSince: null },
    South: { requested: false, crossing: false, duration: 0, waitingSince: null },
    East:  { requested: false, crossing: false, duration: 0, waitingSince: null },
    West:  { requested: false, crossing: false, duration: 0, waitingSince: null }
};
...
let pedestrianCrossingTimeout = null;
...
aedes.on('publish', async (packet, client) => {
    ...
    if (topic.startsWith('traffic/piezo/')) {
        try {
            ...
            if (data.heavyVehicle) {
                piezoData[road] = true;
                console.log(`🚛🚛 HEAVY VEHICLE detected on ${road}! +5s GREEN bonus for next cycle`);
                
                // Clear previous timeout for this road
                if (heavyVehicleTimeout[road]) {
                    clearTimeout(heavyVehicleTimeout[road]);
                }
                
                // Set timeout to clear heavy vehicle status after bonus window
                heavyVehicleTimeout[road] = setTimeout(() => {
                    piezoData[road] = false;
                    console.log(`⏰ Heavy vehicle bonus expired for ${road}`);
                    io.emit('piezoUpdate', { road, heavyVehicle: false });
                }, HEAVY_VEHICLE_BONUS_DURATION);
            }
            
            io.emit('piezoUpdate', { road, heavyVehicle: piezoData[road] });
    ...
    if (topic.startsWith('traffic/pedestrian/')) {
        try {
            ...
            if (data.requested !== undefined && data.requested === true) {
                pedStatus[road].requested = true;
                pedStatus[road].waitingSince = Date.now();
                console.log(`🚶🚶 PEDESTRIAN WAITING at ${road} - Priority increased`);
            }
            ...
            if (data.crossing !== undefined) {
                ...
                if (data.crossing) {
                    console.log(`🚶 PEDESTRIAN CROSSING at ${road} for ${data.duration}s`);
                    // Clear pedestrian waiting status when crossing starts
                    pedStatus[road].requested = false;
                } else {
                    console.log(`✅ Pedestrian crossing finished at ${road}`);
                }
            }
...
function decideNextWinner() {
    ...
    if (latestDecision && latestDecision.winner) {
        ...
        // Check if heavy vehicle bonus should be applied
        let baseGreen = greenTime[winnerRoad] || 5;
        if (piezoData[winnerRoad]) {
            baseGreen += 5; // +5 seconds for heavy vehicle
            console.log(`🚛 Heavy vehicle bonus applied to ${winnerRoad}: +5s (Total: ${baseGreen}s)`);
        }
        latestDecision.greenDuration = baseGreen;
...
function runOneCycle() {
    ...
    let greenDuration = decision.greenDuration || greenTime[winner] || 5;
...
app.get('/api/pedestrian-status', (req, res) => {
    res.json(pedStatus);
});

app.get('/api/heavy-vehicle-status', (req, res) => {
    res.json(piezoData);
});
...
setTimeout(async () => {
    ...
    console.log(`   - GREEN Time: 3s base + traffic bonus (Light: +3s, Heavy: +6s) + Heavy Vehicle: +5s`);
    console.log(`   - PEDESTRIAN: Waiting priority +40, Crossing priority +60`);
