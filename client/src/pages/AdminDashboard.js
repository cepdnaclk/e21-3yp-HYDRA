...
// client/src/pages/AdminDashboard.js — HYDRA v9.0 COLLISION-FREE
//
// DASHBOARD CHANGES from v8.0:
//   - Green border shown on NEXT winner road during post-green yellow (early preview)
//   - Green border shown on CURRENT winner when it is GREEN
//   - "NEXT: X Road" preview badge displayed on next winner's card
//   - Google Traffic shown as driver info ONLY — no scoring label
//   - Priority table reflects new pure sensor-based scoring
//   - Cooldown indicator shown on card that just finished GREEN
//   - System mode shows SENSOR / FALLBACK (no more BOTH/GOOGLE_ONLY)
    ...
    return (
        <div style={{ background: '#0f172a', borderRadius: 10, padding: 12, marginBottom: 8, border: '1px solid #1e293b' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontSize: 14 }}>📡</span>
                <span style={{ fontSize: 11, fontWeight: 'bold', color: '#94a3b8' }}>DUAL ULTRASONIC SENSORS</span>
                <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, fontWeight: 'bold',
                    background: usOnline ? '#14532d' : '#7f1d1d',
                    color: usOnline ? '#4ade80' : '#f87171' }}>
                    {usOnline ? '● ONLINE' : '● OFFLINE'}
                </span>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                {/* US1 */}
                <div style={{ flex: 1, padding: '8px 10px', borderRadius: 8, border: '1px solid',
                    background: us1Stable ? '#3a0000' : '#0f2010',
                    borderColor: us1Stable ? '#ef4444' : '#22c55e' }}>
                    <div style={{ fontSize: 10, color: '#64748b', marginBottom: 4 }}>US1 — 5cm from stop line</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#64748b', marginTop: 4 }}>
                        Raw: {us1Raw < 900 ? `${Number(us1Raw).toFixed(1)} cm` : 'No reading'}
                    </div>
                    {us1Stable && <div style={{ fontSize: 9, color: '#f87171', marginTop: 3 }}>Stable ≥5s → confirmed</div>}
                </div>
                {/* US2 */}
                <div style={{ flex: 1, padding: '8px 10px', borderRadius: 8, border: '1px solid',
                    background: us2Stable && us1Stable ? '#3a0000' : us2Stable && !us1Stable ? '#2a1500' : '#0f2010',
                    borderColor: us2Stable && us1Stable ? '#ef4444' : us2Stable && !us1Stable ? '#f59e0b' : '#22c55e' }}>
                    <div style={{ fontSize: 10, color: '#64748b', marginBottom: 4 }}>US2 — 15cm from stop line</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ width: 12, height: 12, borderRadius: '50%',
                            background: us2Stable && us1Stable ? '#ef4444' : us2Stable ? '#f59e0b' : '#22c55e',
                            boxShadow: us2Stable && us1Stable ? '0 0 8px #ef4444' : 'none' }} />
                    ...
                    <div style={{ fontSize: 10, color: '#64748b', marginTop: 4 }}>
                        Raw: {us2Raw < 900 ? `${Number(us2Raw).toFixed(1)} cm` : 'No reading'}
                    </div>
                    {us2Stable && !us1Stable &&
                        <div style={{ fontSize: 9, color: '#fde047', marginTop: 3 }}>US2 alone = ignored</div>}
                </div>
            </div>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: qlBg, border: `1px solid ${qlColor}44`,
                display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: qlColor, fontWeight: 'bold', fontSize: 12 }}>
                    {ql === 'Heavy' ? '🔴 HEAVY QUEUE' : ql === 'Light' ? '🟡 LIGHT QUEUE' : '🟢 NO QUEUE'}
                </span>
                <span style={{ color: '#64748b', fontSize: 10 }}>
                    {ql === 'Heavy' ? '+6s green (9s total)' : ql === 'Light' ? '+3s green (6s total)' : '3s base green'}
                </span>
            </div>
        </div>
    );
        ...
        <div style={{ background: statusBg, border: `1px solid ${borderColor}`, borderRadius: 10, padding: 12, marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: statusColor, fontWeight: 'bold', marginTop: 4 }}>{statusLabel}</div>
                    {isCrossing && <div style={{ fontSize: 10, color: '#60a5fa', marginTop: 4 }}>Car signal held RED</div>}
                    {isWaiting && phase === 'GREEN' && <div style={{ fontSize: 10, color: '#fde047', marginTop: 4 }}>Will cross after GREEN + YELLOW finish</div>}
                    {isWaiting && phase === 'RED' && <div style={{ fontSize: 10, color: '#fde047', marginTop: 4 }}>Crossing when remaining time &gt; 3s</div>}
                    {isWaiting && phase === 'YELLOW' && <div style={{ fontSize: 10, color: '#fde047', marginTop: 4 }}>Countdown shown, crossing soon</div>}
                    {!isCrossing && !isWaiting && <div style={{ fontSize: 10, color: '#475569', marginTop: 4 }}>No pedestrian waiting</div>}
                </div>
                {isCrossing && ped.duration > 0 && (
                    <div style={{ background: '#14532d', color: '#4ade80', width: 44, height: 44, borderRadius: '50%',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 'bold', flexShrink: 0 }}>
                        {ped.duration}
                    </div>
                )}
            </div>
...
// ── Rain Panel ────────────────────────────────────────────────────────────────
const RainPanel = ({ rainDetected, yellowTime, isNorthRoad }) => (
    <div style={{ background: rainDetected ? '#0d1f3d' : '#0f172a',
        border: `1px solid ${rainDetected ? '#3b82f6' : '#1e293b'}`,
        borderRadius: 10, padding: 12, marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 20 }}>{rainDetected ? '🌧️' : '☀️'}</span>
            <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: '#64748b', marginTop: 2 }}>
                    Yellow light: <strong style={{ color: rainDetected ? '#fde047' : '#94a3b8' }}>
                        {yellowTime}s {rainDetected ? '(3s + 2s rain extension)' : '(normal)'}
                    </strong>
                </div>
            </div>
            <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 6,
                background: isNorthRoad ? '#1e3a5f' : '#1e293b',
                color: isNorthRoad ? '#60a5fa' : '#64748b' }}>
                {isNorthRoad ? '📡 Sensor here' : 'from North'}
            </span>
        </div>
    </div>
);

// ── Queue level label helper ──────────────────────────────────────────────────
function queueLevelLabel(us1Stable, us2Stable, piezoHeavy) {
...
export default function AdminDashboard({ user, onLogout }) {
    ...
    useEffect(() => {
        ...
        socket.on('fullState', data => {
            ...
            if (data.espOnline)     setEspOnline(data.espOnline);
            if (data.latestDecision) setDecision(data.latestDecision);
            if (data.nextWinnerRoad !== undefined) setNextWinner(data.nextWinnerRoad);
        });
        ...
        socket.on('ledStateUpdate',   ({ road, state }) => setLivePhase(p => ({ ...p, [road]: state })));
        socket.on('newDecision',      dec => setDecision(dec));
        socket.on('nextWinnerPreview',({ nextWinner: nw }) => setNextWinner(nw));
        socket.on('usUpdate',         ({ road, us1Stable, us2Stable, us1Raw, us2Raw }) => {
            setUsData(prev => ({ ...prev, [road]: { us1Stable, us2Stable, us1Raw, us2Raw } }));
            setUsWorking(prev => ({ ...prev, [road]: true }));
        });
    ...
    return (
        <div style={{ padding: 20, fontFamily: "'Segoe UI', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: 'white' }}>
            ...
            <div style={{ textAlign:'center', marginBottom: 24 }}>
                <h1 style={{ fontSize:'2rem', margin:'0 0 4px', letterSpacing:2 }}>🚦 H.Y.D.R.A Admin Dashboard</h1>
                <p style={{ color:'#475569', margin:0, fontSize:13 }}>Nawinna Junction — Sensor-Based Priority Control v9.0</p>
                <div style={{ marginTop:10, display:'flex', justifyContent:'center', gap:10, flexWrap:'wrap', alignItems:'center' }}>
                    <span style={{ background:connected?'#14532d':'#7f1d1d', color:connected?'#4ade80':'#f87171',
                        padding:'3px 12px', borderRadius:20, fontSize:12, fontWeight:'bold' }}>
                        {connected ? '● LIVE' : '● OFFLINE'}
                    ...
                    {nextWinner && (
                        <span style={{ background:'#1a2e1a', color:'#4ade80', border:'1px solid #22c55e44',
                            padding:'3px 12px', borderRadius:20, fontSize:12, fontWeight:'bold' }}>
                            ⏭️ NEXT: {nextWinner} Road
                        </span>
                    )}
                </div>
            </div>
                    ...
                    <div style={{ display:'flex', gap:16, alignItems:'center', flexWrap:'wrap' }}>
                        ...
                        <div style={{ flex:1 }}>
                            ...
                            <div style={{ color:'#94a3b8', fontSize:12, marginTop:4 }}>
                                ...
                                🔴 Others RED: <strong style={{ color:'#f87171' }}>{decision.redForOthers}s</strong>
                                &nbsp;| Mode: {decision.mode}
                                {nextWinner && <span style={{ color:'#4ade80', marginLeft:12 }}>⏭ Next: {nextWinner}</span>}
                            </div>
                            {rainDetected && (
                                <div style={{ color:'#60a5fa', fontSize:11, marginTop:4 }}>
                                    🌧️ Rain — Yellow extended to {yellowTime}s (+2s safety)
                                </div>
                            )}
                        </div>
                        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(440px, 1fr))', gap:16, marginBottom:22 }}>
                {ROADS.map(road => {
                    ...
                    const ped        = pedStatus[road] || { requested:false, crossing:false, duration:0 };
                    const espUp      = espOnline[road] === true;
                    const us         = usData[road] || { us1Stable:false, us2Stable:false, us1Raw:999, us2Raw:999 };
                    const piezo      = (piezoData[road] || {}).heavy === true;
                    const google     = googleTraffic[road] || 'Unknown';
                    const phaseColor = phase==='GREEN'?'#4ade80':phase==='YELLOW'?'#fde047':'#f87171';
                    const greenBreak = queueLevelLabel(us.us1Stable, us.us2Stable, piezo);

                    return (
                        <div key={road} style={{
                            background:'linear-gradient(160deg,#1a2540,#111827)',
                            borderRadius:16, padding:18,
                            border: showGreenBorder ? '2px solid #22c55e'
                                  : !espUp ? '1px solid #7f1d1d'
                                  : '1px solid #1e3a5f',
                            boxShadow: showGreenBorder ? '0 0 24px rgba(34,197,94,0.18)' : 'none',
                            transition:'all 0.3s'
                        }}>
                            {/* Road header */}
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, flexWrap:'wrap' }}>
                                ...
                                {isWinner && phase === 'GREEN' && (
                                    <span style={{ color:'#4ade80', fontSize:11, fontWeight:'bold' }}>● PRIORITY</span>
                                )}
                                {isNextWin && (
                                    <span style={{ background:'#14532d44', color:'#4ade80', border:'1px solid #22c55e',
                                        padding:'2px 8px', borderRadius:8, fontSize:10, fontWeight:'bold' }}>
                                        ⏭ NEXT GREEN
                                    </span>
                                )}
                                {!espUp && (
                                    <span style={{ background:'#7f1d1d', color:'#f87171', border:'1px solid #ef4444',
                                        padding:'2px 8px', borderRadius:8, fontSize:10, fontWeight:'bold' }}>
                                        ⚡ ESP32 OFFLINE
                                    </span>
                                )}
                                {piezo && (
                                    <span style={{ background:'#1a1000', color:'#fb923c', border:'1px solid #f59e0b',
                                        padding:'2px 8px', borderRadius:8, fontSize:10, fontWeight:'bold' }}>
                                        🚛 HEAVY VEHICLE
                                    </span>
                                )}
                            ...
                            <div style={{ display:'flex', gap:12, marginBottom:14, alignItems:'center',
                                        ...
                                        <div style={{ position:'relative' }}>
                                            ...
                                            {phase===c && count>0 && (
                                                <div style={{ position:'absolute', inset:0, display:'flex',
                                                    alignItems:'center', justifyContent:'center',
                                                    fontSize:11, fontWeight:'bold', color:'#000' }}>{count}</div>
                                            )}
                                        </div>
                                        <span style={{ fontSize:9, letterSpacing:1,
                                            color: phase===c ? (c==='RED'?'#ef4444':c==='YELLOW'?'#f59e0b':'#22c55e') : '#334155' }}>
                                ...
                                <div style={{ marginLeft:8 }}>
                                    ...
                                    {!isWinner && phase==='RED' && redOthers>0 && (
                                        <div style={{ fontSize:10, color:'#64748b', marginTop:2 }}>
                                            RED for {redOthers}s this cycle
                                        </div>
                                    )}
                            ...
                            {/* Ultrasonic sensor panel */}
                            <USSensorPanel
                                us1Stable={us.us1Stable} us2Stable={us.us2Stable}
                                us1Raw={us.us1Raw} us2Raw={us.us2Raw}
                                usOnline={usWorking[road] && espUp}
                            />

                            {/* Piezo sensor */}
                            <div style={{ background: piezo ? '#1a1000' : '#0f172a',
                                border: `1px solid ${piezo ? '#f59e0b' : '#1e293b'}`,
                                borderRadius:10, padding:10, marginBottom:8 }}>
                                <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                                    <span style={{ fontSize:16 }}>🚛</span>
                                    <span style={{ fontSize:11, color:'#64748b', fontWeight:'bold' }}>PIEZO VIBRATION SENSOR</span>
                                    <span style={{ fontSize:10, padding:'2px 8px', borderRadius:6, fontWeight:'bold',
                                        background: piezo ? '#3d2000' : '#1e293b',
                                        color: piezo ? '#fb923c' : '#475569',
                                        border: `1px solid ${piezo ? '#f59e0b' : '#334155'}` }}>
                                        {piezo ? '● HEAVY VEHICLE DETECTED' : '● IDLE'}
                                ...
                                <div style={{ fontSize:10, color:'#64748b', marginTop:6 }}>
                                    {piezo
                                        ? 'Heavy vehicle confirmed (US1 + vibration) → +3s green bonus'
                                        : 'US1 must be stable to confirm. Monitoring for vibration.'}
                                </div>
                            </div>
                            ...
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8,
                                background:'#0f172a', padding:'8px 10px', borderRadius:8 }}>
                                <span style={{ fontSize:14 }}>🗺️</span>
                                <span style={{ fontSize:11, color:'#64748b' }}>Next intersection (driver info):</span>
                                <span style={{ padding:'2px 10px', borderRadius:10, fontSize:11, fontWeight:'bold',
                                    background: google==='Heavy'?'#7f1d1d':google==='Medium'?'#713f12':'#14532d',
                                    color: google==='Heavy'?'#f87171':google==='Medium'?'#fde047':'#4ade80' }}>
                                    {google}
                                </span>
                                <span style={{ fontSize:9, color:'#334155', fontStyle:'italic' }}>info only</span>
                            </div>
            ...
            {decision?.priorities && (
                <div style={{ background:'linear-gradient(160deg,#1a2540,#111827)', borderRadius:16, padding:20, marginBottom:22, border:'1px solid #1e3a5f' }}>
                    <h3 style={{ margin:'0 0 14px', color:'#94a3b8', fontSize:14 }}>📋 Signal Priority Analysis (Sensor-Based)</h3>
                    <div style={{ overflowX:'auto' }}>
                        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                            <thead>
                                <tr style={{ borderBottom:'1px solid #1e3a5f' }}>
                                    {['Rank','Road','US1','US2','Queue','Piezo (+3s)','Score','Green','Status'].map(h => (
                                        <th key={h} style={{ padding:'8px 10px', textAlign:'left', color:'#475569', fontSize:10, whiteSpace:'nowrap' }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                                ...
                                {decision.priorities.map((p, i) => {
                                    ...
                                    return (
                                        <tr key={p.road} style={{ borderBottom:'1px solid #0f172a',
                                            background: i===0 ? 'rgba(34,197,94,0.05)'
                                                      : isNext ? 'rgba(34,197,94,0.03)'
                                                      : 'transparent' }}>
                                            <td style={{ padding:'8px 10px', color:i===0?'#4ade80':'#64748b', fontWeight:'bold' }}>#{i+1}</td>
                                            <td style={{ padding:'8px 10px', fontWeight:'bold', color:i===0?'#e2e8f0':'#94a3b8' }}>
                                                {p.road}
                                                {isNext && <span style={{ marginLeft:6, fontSize:9, color:'#4ade80' }}>⏭NEXT</span>}
                                            </td>
                                            <td style={{ padding:'8px 10px', color:p.us1Stable?'#f87171':'#4ade80', fontWeight:'bold' }}>
                                                {p.us1Stable ? '🔴 STABLE' : '🟢 clear'}
                                            </td>
                                            <td style={{ padding:'8px 10px', color:p.us2Stable&&p.us1Stable?'#f87171':p.us2Stable?'#fde047':'#4ade80', fontWeight:'bold' }}>
                                                {p.us2Stable ? (p.us1Stable ? '🔴 STABLE' : '⚠️ IGNORED') : '🟢 clear'}
                                            </td>
                                            <td style={{ padding:'8px 10px', color:p.queueLevel==='Heavy'?'#f87171':p.queueLevel==='Light'?'#fde047':'#4ade80', fontWeight:'bold' }}>
                                                {p.queueLevel}
                                            </td>
                                            <td style={{ padding:'8px 10px', color:p.piezoHeavy?'#fb923c':'#475569', fontWeight:'bold' }}>
                                                {p.piezoHeavy ? '🚛 +3s' : '—'}
                                            </td>
                                            <td style={{ padding:'8px 10px', color:p.score>0?'#4ade80':p.score<0?'#f87171':'#94a3b8', fontWeight:'bold' }}>
                    ...
                    <div style={{ marginTop:10, padding:10, background:'#0f172a', borderRadius:8, fontSize:11, color:'#64748b', border:'1px solid #1e3a5f' }}>
                        🔴 <strong style={{ color:'#94a3b8' }}>Dynamic RED:</strong>&nbsp;
                        {winner && decision?.greenDuration
                            ? `2s pre-yellow + ${decision.greenDuration}s green + ${decision.yellowDuration}s yellow = ${decision.redForOthers}s total`
                            : 'Calculated each cycle'}
                        &nbsp;|&nbsp;
                        <span style={{ color:'#475569' }}>Google Traffic shown for driver information only — not used in signal priority</span>
                    </div>
                </div>
            )}
            ...
            {/* System rules */}
            <div style={{ background:'linear-gradient(160deg,#1a2540,#111827)', borderRadius:16, padding:16, marginBottom:22, border:'1px solid #1e3a5f' }}>
                <h3 style={{ margin:'0 0 12px', color:'#94a3b8', fontSize:13 }}>📋 System Rules (v9.0 Sensor-Based)</h3>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px,1fr))', gap:10, fontSize:11, color:'#64748b' }}>
                    <div>📡 <strong style={{ color:'#60a5fa' }}>US1 (5cm)</strong>: &lt;7cm for 5s → vehicle confirmed at stop line</div>
                    <div>📡 <strong style={{ color:'#60a5fa' }}>US2 (15cm)</strong>: valid ONLY when US1 also stable (queue length)</div>
                    <div>🟢 <strong>GREEN</strong>: 3s base | US1→+3s=6s | US1+US2→+6s=9s | +Piezo→+3s each</div>
                    ...
                    <div>🚶 <strong>Pedestrian</strong>: RED=cross now | GREEN=wait | YELLOW=countdown+cross</div>
                    <div>🗺️ <strong>Google Traffic</strong>: driver info display ONLY — does NOT affect signal timing</div>
                </div>
            </div>
            ...
            <div style={{ background:'linear-gradient(160deg,#1a2540,#111827)', borderRadius:16, padding:20, border:'1px solid #1e3a5f' }}>
                <h3 style={{ margin:'0 0 6px', color:'#e2e8f0', fontSize:16 }}>🗺️ Traffic Analytics — Nawinna Junction</h3>
                <p style={{ color:'#475569', fontSize:12, margin:'0 0 16px' }}>
                    Real-time data to help road users choose the best time and route.
                </p>
                <div style={{ display:'flex', gap:8, marginBottom:18, flexWrap:'wrap' }}>
                    {[
                        { id:'livecongestion', label:'🚦 Live Road Status' },
                    ...
                    <div>
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px,1fr))', gap:12 }}>
                            {ROADS.map(road => {
                                ...
                                let cong='Low', waitEst='Under 1 min', tip='Good to travel', barColor='#22c55e';
                                if (!espUp)           { cong='Unknown'; waitEst='Sensor offline'; tip='Proceed with caution'; barColor='#64748b'; }
                                else if (ql==='Heavy') { cong='Heavy'; waitEst='Expect delays'; tip='Consider alternate route'; barColor='#ef4444'; }
                                else if (ql==='Light') { cong='Moderate'; waitEst='~6-9s wait'; tip='Some traffic — normal wait'; barColor='#f59e0b'; }
                                return (
                                    <div key={road} style={{ background:'#0f172a', borderRadius:12, padding:14, border:`2px solid ${barColor}44` }}>
                                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                                            <span style={{ fontWeight:'bold', color:'#e2e8f0', fontSize:14 }}>{road} Road</span>
                                        ...
                                        <div style={{ fontSize:11, color:'#64748b', marginBottom:6 }}>{tip}</div>
                                        {google !== 'Unknown' && (
                                            <div style={{ fontSize:10, color:'#475569' }}>
                                                Next junction: <span style={{ color: google==='Heavy'?'#f87171':google==='Medium'?'#fde047':'#4ade80' }}>{google}</span>
                                                <span style={{ color:'#334155', fontStyle:'italic' }}> (info)</span>
                                            </div>
                                        )}
                                        <div style={{ background:'#1e293b', borderRadius:4, height:6, marginTop:8 }}>
                                            <div style={{ width:cong==='Heavy'?'85%':cong==='Moderate'?'50%':cong==='Unknown'?'30%':'15%',
                ...
                {analyticsTab === 'besttimes' && (
                    <div>
                        <p style={{ fontSize:11, color:'#64748b', marginBottom:14 }}>
                            Historical data. Shows least congested hours at this junction.
                        </p>
                        {analyticsData.peakHours && analyticsData.peakHours.filter(h=>h.North>0||h.South>0||h.East>0||h.West>0).length>0 ? (
                            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(110px,1fr))', gap:8 }}>
                                {analyticsData.peakHours.filter(h=>h.North>0||h.South>0||h.East>0||h.West>0).map(h=>{
                ...
                {analyticsTab === 'roadhealth' && (
                    <div>
                        <p style={{ fontSize:11, color:'#64748b', marginBottom:14 }}>
                            Which road gets the most green priority and average wait times.
                        </p>
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:12 }}>
                            {analyticsData.roadPerf && analyticsData.roadPerf.length > 0 ? analyticsData.roadPerf.map(r => {
                                const wait=r.avgWaitTime, color=wait>30?'#ef4444':wait>15?'#f59e0b':'#22c55e';
            ...
            <div style={{ textAlign:'center', marginTop:28, color:'#1e3a5f', fontSize:11 }}>
                HYDRA v9.0 — Admin Dashboard — Sensor-Based Priority — Nawinna Junction, Kurunegala
            </div>
        </div>
    );
}