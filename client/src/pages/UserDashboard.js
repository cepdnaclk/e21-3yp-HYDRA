...
// client/src/pages/UserDashboard.js — HYDRA v9.0 USER VERSION
// Same as AdminDashboard but WITHOUT Force Override panel and System Rules section.
// Green border logic: shows on current winner AND on nextWinner (preview).
...
const PedBulb = ({ color, active, size = 26 }) => {
    const C = { RED: { on: '#ef4444', off: '#3a0000' }, GREEN: { on: '#22c55e', off: '#003310' } };
    const c = C[color];
    return (
        <div style={{
            width: size, height: size, borderRadius: '50%',
        ...
        <div style={{ background:'#0f172a', borderRadius:10, padding:12, marginBottom:8, border:'1px solid #1e293b' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
                ...
                <span style={{ fontSize:9, padding:'2px 8px', borderRadius:10, fontWeight:'bold',
                    background: usOnline?'#14532d':'#7f1d1d', color: usOnline?'#4ade80':'#f87171' }}>
                    {usOnline ? '● ONLINE' : '● OFFLINE'}
                </span>
            </div>
            <div style={{ display:'flex', gap:8, marginBottom:10 }}>
                <div style={{ flex:1, padding:'8px 10px', borderRadius:8, border:'1px solid',
                    background: us1Stable?'#3a0000':'#0f2010', borderColor: us1Stable?'#ef4444':'#22c55e' }}>
                    <div style={{ fontSize:10, color:'#64748b', marginBottom:4 }}>US1 — 5cm from stop line</div>
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                        <div style={{ width:12, height:12, borderRadius:'50%', background: us1Stable?'#ef4444':'#22c55e',
                ...
                <div style={{ flex:1, padding:'8px 10px', borderRadius:8, border:'1px solid',
                    background: us2Stable&&us1Stable?'#3a0000':us2Stable?'#2a1500':'#0f2010',
                    borderColor: us2Stable&&us1Stable?'#ef4444':us2Stable?'#f59e0b':'#22c55e' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:'bold',
                            ...
                            {us2Stable&&us1Stable?'🔴 QUEUE BACKED UP':us2Stable?'⚠️ IGNORED':'🟢 CLEAR'}
                        </span>
                    </div>
                    <div style={{ fontSize:10, color:'#64748b', marginTop:4 }}>
...
const PedestrianPanel = ({ ped, phase }) => {
    ...
    if (isCrossing) { statusColor='#60a5fa'; statusLabel=`CROSSING (${ped.duration||0}s left)`; statusBg='#1e3a5f'; borderColor='#3b82f6'; }
    else if (isWaiting) { statusColor='#fde047'; statusLabel=`WAITING — pressed during ${phase}`; statusBg='#3d2000'; borderColor='#f59e0b'; }
    return (
        <div style={{ background:statusBg, border:`1px solid ${borderColor}`, borderRadius:10, padding:12, marginBottom:8 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6,
...
const RainPanel = ({ rainDetected, yellowTime, isNorthRoad }) => (
    <div style={{ background:rainDetected?'#0d1f3d':'#0f172a', border:`1px solid ${rainDetected?'#3b82f6':'#1e293b'}`,
        borderRadius:10, padding:12, marginBottom:8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ fontSize:20 }}>{rainDetected?'🌧️':'☀️'}</span>
            <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:10, color:'#64748b', marginTop:2 }}>
                    Yellow light: <strong style={{ color:rainDetected?'#fde047':'#94a3b8' }}>
                        {yellowTime}s {rainDetected?'(+2s rain)':'(normal)'}
                    </strong>
                </div>
            </div>
            <span style={{ fontSize:9, padding:'2px 6px', borderRadius:6,
                background:isNorthRoad?'#1e3a5f':'#1e293b', color:isNorthRoad?'#60a5fa':'#64748b' }}>
...
export default function UserDashboard({ user, onLogout }) {
    ...
    useEffect(() => {
        ...
        socket.on('countdown',        ({ road, phase, remaining }) => { setLiveCD(p=>({...p,[road]:remaining})); setLivePhase(p=>({...p,[road]:phase})); });
        socket.on('ledStateUpdate',   ({ road, state }) => setLivePhase(p=>({...p,[road]:state})));
        socket.on('newDecision',      dec => setDecision(dec));
        ...
        socket.on('piezoUpdate',      ({ road, heavyVehicle }) => setPiezoData(prev=>({...prev,[road]:{...prev[road],heavy:heavyVehicle}})));
        socket.on('rainUpdate',       ({ rainDetected: r }) => { setRainDetected(r); setYellowTime(r?5:3); });
        socket.on('pedestrianUpdate', ({ road, ...rest }) => setPedStatus(p=>({...p,[road]:rest})));
        ...
        <div style={{ padding:20, fontFamily:"'Segoe UI', sans-serif", background:'#0a0f1e', minHeight:'100vh', color:'white' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    ...
                    <div>
                        <div style={{ color:'#e2e8f0', fontSize:13, fontWeight:'bold' }}>{user?.name || user?.email}</div>
                        <div style={{ color:'#475569', fontSize:11 }}>{user?.email}</div>
                    </div>
                </div>
                        ...
                        <div key={road} style={{
                            ...
                            <div style={{ display:'flex', gap:12, marginBottom:14, alignItems:'center',
                                ...
                                <div style={{ marginLeft:8 }}>
                                    ...
                                    <div style={{ fontSize:10, color:'#60a5fa', marginTop:4 }}>🟢 {greenBreak}</div>
                                </div>
                            </div>
                            ...
                            {/* Piezo */}
                            <div style={{ background:piezo?'#1a1000':'#0f172a', border:`1px solid ${piezo?'#f59e0b':'#1e293b'}`,
                                borderRadius:10, padding:10, marginBottom:8 }}>
                                <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                                    <span style={{ fontSize:16 }}>🚛</span>
                                    <span style={{ fontSize:11, color:'#64748b', fontWeight:'bold' }}>PIEZO VIBRATION SENSOR</span>
                                    <span style={{ fontSize:10, padding:'2px 8px', borderRadius:6, fontWeight:'bold',
                                        background:piezo?'#3d2000':'#1e293b', color:piezo?'#fb923c':'#475569',
                                        border:`1px solid ${piezo?'#f59e0b':'#334155'}` }}>
                                        {piezo?'● HEAVY VEHICLE DETECTED':'● IDLE'}
                                    </span>
                                </div>
                                <div style={{ fontSize:10, color:'#64748b', marginTop:6 }}>
                                    {piezo?'Heavy vehicle confirmed (US1 + vibration) → +3s green bonus'
                                          :'US1 must be stable to confirm. Monitoring.'}
                                </div>
                            </div>

                            <RainPanel rainDetected={rainDetected} yellowTime={yellowTime} isNorthRoad={road==='North'} />

                            {/* Google Traffic — display only */}
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8,
                                background:'#0f172a', padding:'8px 10px', borderRadius:8 }}>
                                <span style={{ fontSize:14 }}>🗺️</span>
                            ...
                            <PedestrianPanel ped={ped} phase={phase} />
                            {/* No Force Override panel for user view */}
                        </div>
                    );
                })}
            </div>
                ...
                <div style={{ background:'linear-gradient(160deg,#1a2540,#111827)', borderRadius:16, padding:20, marginBottom:22, border:'1px solid #1e3a5f' }}>
                    ...
                    <div style={{ overflowX:'auto' }}>
                        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                            <thead>
                                <tr style={{ borderBottom:'1px solid #1e3a5f' }}>
                                    {['Rank','Road','US1','US2','Queue','Piezo','Score','Green','Status'].map(h=>(
                                        <th key={h} style={{ padding:'8px 10px', textAlign:'left', color:'#475569', fontSize:10, whiteSpace:'nowrap' }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                                ...
                                {decision.priorities.map((p,i)=>{
                                    ...
                                    return (
                                        <tr key={p.road} style={{ borderBottom:'1px solid #0f172a',
                                            background:i===0?'rgba(34,197,94,0.05)':isNext?'rgba(34,197,94,0.03)':'transparent' }}>
                                            <td style={{ padding:'8px 10px', color:i===0?'#4ade80':'#64748b', fontWeight:'bold' }}>#{i+1}</td>
                                            <td style={{ padding:'8px 10px', fontWeight:'bold', color:i===0?'#e2e8f0':'#94a3b8' }}>
                                                {p.road}{isNext&&<span style={{marginLeft:6,fontSize:9,color:'#4ade80'}}>⏭NEXT</span>}
                                            </td>
                                            ...
                                            <td style={{ padding:'8px 10px', color:p.onCooldown?'#64748b':p.score>0?'#4ade80':p.score<0?'#f87171':'#94a3b8', fontWeight:'bold' }}>
                                                {p.onCooldown?<span style={{color:'#64748b'}}>COOLDOWN</span>:(typeof p.score==='number'?p.score:'—')}
                                            </td>
                                            <td style={{ padding:'8px 10px', color:i===0?'#4ade80':'#94a3b8', fontWeight:i===0?'bold':'normal' }}>
            ...
            <div style={{ background:'linear-gradient(160deg,#1a2540,#111827)', borderRadius:16, padding:20, border:'1px solid #1e3a5f' }}>
                <h3 style={{ margin:'0 0 6px', color:'#e2e8f0', fontSize:16 }}>🗺️ Traffic Analytics — Nawinna Junction</h3>
                <p style={{ color:'#475569', fontSize:12, margin:'0 0 16px' }}>Live data to help you choose the best time and route.</p>
                <div style={{ display:'flex', gap:8, marginBottom:18, flexWrap:'wrap' }}>
                    {[{id:'livecongestion',label:'🚦 Live Road Status'},{id:'besttimes',label:'⏰ Best Times'},{id:'roadhealth',label:'🛣️ Performance'}].map(tab=>(
                        <button key={tab.id} onClick={()=>setAnalyticsTab(tab.id)} style={{
                            background:analyticsTab===tab.id?'#1e3a5f':'#0f172a',
                            color:analyticsTab===tab.id?'#60a5fa':'#475569',
                            border:`1px solid ${analyticsTab===tab.id?'#3b82f6':'#334155'}`,
                    ...
                    <div>
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:12 }}>
                            {ROADS.map(road=>{
                                ...
                                else if (ql==='Light'){cong='Moderate';waitEst='~6-9s wait';tip='Some traffic';barColor='#f59e0b';}
                                return (
                                    <div key={road} style={{ background:'#0f172a', borderRadius:12, padding:14, border:`2px solid ${barColor}44` }}>
                                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                                            <span style={{ fontWeight:'bold', color:'#e2e8f0', fontSize:14 }}>{road} Road</span>
                        ...
                        {rainDetected && (
                            <div style={{ marginTop:12, padding:12, background:'#0f1f3d', border:'1px solid #3b82f6', borderRadius:10 }}>
                                <div style={{ color:'#60a5fa', fontWeight:'bold', fontSize:13 }}>🌧️ Rain Advisory</div>
                                <div style={{ color:'#94a3b8', fontSize:12, marginTop:4 }}>
                                    Rain detected. Yellow extended to {yellowTime}s. Allow extra braking distance.
                                </div>
                            </div>
                        )}
                    </div>
                                ...
                                {analyticsData.peakHours.filter(h=>h.North>0||h.South>0||h.East>0||h.West>0).map(h=>{
                                    ...
                                    return (
                                        <div key={h.hour} style={{ background:'#0f172a', borderRadius:8, padding:10, border:`1px solid ${color}33`, textAlign:'center' }}>
                                            <div style={{ fontSize:14, fontWeight:'bold', color:'#e2e8f0' }}>{h.hour.toString().padStart(2,'0')}:00</div>
                                            <div style={{ fontSize:11, color, fontWeight:'bold', marginTop:3 }}>{avg>60?'Peak':avg>30?'Moderate':'✅ Good'}</div>
                                            <div style={{ background:'#1e293b', borderRadius:3, height:5, marginTop:5 }}>
                                                <div style={{ width:`${avg}%`, background:color, height:'100%', borderRadius:3 }} />
                                            </div>
                                        </div>
                ...
                {analyticsTab==='roadhealth' && (
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:12 }}>
                        {analyticsData.roadPerf&&analyticsData.roadPerf.length>0?analyticsData.roadPerf.map(r=>{
                            ...
                            <div style={{ color:'#475569', fontSize:12, padding:20 }}>Collecting data...</div>
                        )}
                    </div>
                )}
            </div>
            ...
            <div style={{ textAlign:'center', marginTop:28, color:'#1e3a5f', fontSize:11 }}>
                HYDRA v9.0 — User Dashboard — Nawinna Junction, Kurunegala
            </div>
        </div>
    );
}