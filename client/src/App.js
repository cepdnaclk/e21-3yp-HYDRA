...
// client/src/App.js — HYDRA Dashboard v5.1 — Fixed Offline Road Badges
import React, { useState, useEffect, useRef } from 'react';
import io from 'socket.io-client';
import axios from 'axios';
...
// ─────────────────────────────────────────────────────────────────────────────
// SCENARIO BADGE — updated to handle all cases including offline roads
// IR / ULTRASONIC (only for North with working ESP32)
// GOOGLE_ONLY (ESP32 offline but Google works)
// NO_DATA (both offline)
// FALLBACK (system-wide fallback)
// ─────────────────────────────────────────────────────────────────────────────
const ScenarioBadge = ({ scenario }) => {
    const map = {
        IR:          { bg: '#3b1f69', color: '#c084fc', border: '#a855f7', icon: '🔦', label: 'IR MODE' },
        ULTRASONIC:  { bg: '#1e3a5f', color: '#60a5fa', border: '#3b82f6', icon: '📡', label: 'ULTRASONIC' },
        GOOGLE_ONLY: { bg: '#1a2e1a', color: '#4ade80', border: '#22c55e', icon: '🗺️', label: 'GOOGLE ONLY' },
        NO_DATA:     { bg: '#2d1515', color: '#f87171', border: '#ef4444', icon: '❌', label: 'NO DATA' },
        FALLBACK:    { bg: '#3d2000', color: '#fb923c', border: '#f59e0b', icon: '⚠️', label: 'FALLBACK' },
    };
    const s = map[scenario] || map.NO_DATA;
    return (
        <span style={{
            background: s.bg, color: s.color, border: `1px solid ${s.border}`,
            padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 'bold',
...
function App() {
    ...
    // Helper: determine what data source label to show in the sensor section
    const getSensorStatusLabel = (road, scenario, espUp) => {
        if (!espUp) return { text: 'ESP32 OFFLINE', color: '#f87171', bg: '#7f1d1d' };
        if (scenario === 'GOOGLE_ONLY') return { text: 'GOOGLE ONLY', color: '#4ade80', bg: '#14532d' };
        if (scenario === 'NO_DATA') return { text: 'NO DATA', color: '#f87171', bg: '#7f1d1d' };
        if (scenario === 'IR') return { text: 'IR ACTIVE', color: '#c084fc', bg: '#3b1f69' };
        return { text: 'ACTIVE', color: '#4ade80', bg: '#14532d' };
    };
        ...
        <div style={{ padding: 20, fontFamily: "'Segoe UI', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: 'white' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
                ...
                <div style={{ marginTop: 10, display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ background: '#1e293b', color: '#94a3b8', padding: '3px 12px', borderRadius: 20, fontSize: 12 }}>
                        📡 ESP32 Online: {Object.values(espOnline).filter(Boolean).length}/4
                    </span>
                    <span style={{
                        background: googleWorking ? '#14532d' : '#1e293b',
                        color: googleWorking ? '#4ade80' : '#64748b',
                        padding: '3px 12px', borderRadius: 20, fontSize: 12
                    }}>
                        🗺️ Google: {googleWorking ? 'Active' : 'Offline'}
                    </span>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(400px,1fr))', gap: 16, marginBottom: 22 }}>
                {ROADS.map(road => {
                    ...
                    // Determine if sensors are actually usable for this road
                    const hasWorkingSensor = espUp && sensorWorking[road] === true;
                    const sensorLabel = getSensorStatusLabel(road, scenario, espUp);

                    return (
                        <div key={road} style={{
                            background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 18,
                            border: isWin ? '2px solid #22c55e' : espUp ? '1px solid #1e3a5f' : '1px solid #3d1515',
                            boxShadow: isWin ? '0 0 24px rgba(34,197,94,0.18)' : !espUp ? '0 0 12px rgba(239,68,68,0.06)' : 'none',
                            transition: 'all 0.3s'
                        }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                                        ...
                                        {isWin && <span style={{ color: '#4ade80', fontSize: 11, fontWeight: 'bold' }}>● PRIORITY</span>}
                                        {/* Show scenario badge — NO_DATA or GOOGLE_ONLY for offline roads */}
                                        {scenario && <ScenarioBadge scenario={scenario} />}
                                        {/* ESP32 offline badge */}
                                        {!espUp && (
                                            <span style={{ background: '#7f1d1d', color: '#f87171', border: '1px solid #ef4444', padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 'bold' }}>
                                    ...
                                    {/* ── SENSOR SECTION — content depends on ESP32 status ── */}
                                    <div style={{
                                        background: '#0f172a', borderRadius: 8, padding: 10, marginBottom: 8,
                                        border: `1px solid ${espUp ? '#1e3a5f' : '#3d1515'}`
                                    }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                                            <span>{espUp ? '📡' : '🔌'}</span>
                                            <span style={{ fontSize: 11, color: '#64748b' }}>ESP32 Sensors</span>
                                            <span style={{
                                                fontSize: 10, padding: '1px 6px', borderRadius: 6,
                                                background: sensorLabel.bg,
                                                color: sensorLabel.color,
                                                fontWeight: 'bold', border: `1px solid ${sensorLabel.color}44`
                                            }}>
                                                ● {sensorLabel.text}
                                            </span>
                                        </div>
                                        {espUp && hasWorkingSensor ? (
                                            <div style={{ fontSize: 11, color: '#64748b' }}>
                                                Distance: <strong style={{ color: dist >= 5000 ? '#475569' : dist < 20 ? '#ef4444' : dist < 100 ? '#f59e0b' : '#4ade80' }}>
                                                    {dist >= 5000 ? 'No vehicle' : `${Math.round(dist)} cm`}
                                                </strong>
                                                {scenario && (
                                                    <span style={{ marginLeft: 8, color: '#64748b' }}>
                                                        → {scenario === 'IR' ? 'Using IR sensors (dist < 20cm)' : 'Using ultrasonic distance'}
                                                    </span>
                                                )}
                                            </div>
                                        ) : (
                                            <div style={{ fontSize: 11, color: '#64748b', fontStyle: 'italic' }}>
                                                {espUp ? 'Sensor initialising...' : 'No ESP32 connected to this road — no distance, IR or piezo data available.'}
                                            </div>
                                        )}
                                    </div>

                                    {/* ── IR SENSORS — only show data if ESP32 is online ── */}
                                    <div style={{
                                        background: '#0f172a', borderRadius: 8, padding: 10, marginBottom: 8,
                                        border: `1px solid ${espUp ? '#1e3a5f' : '#3d1515'}`
                                    }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                                            <span>🔦</span>
                                            <span style={{ fontSize: 11, color: '#64748b' }}>IR Sensors</span>
                                            {!espUp && <span style={{ fontSize: 10, color: '#f87171', fontWeight: 'bold' }}>OFFLINE</span>}
                                        </div>
                                        {espUp ? (
                                            <>
                                                <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                                            ...
                                            <div style={{ fontSize: 11, color: '#475569', fontStyle: 'italic' }}>
                                                IR data unavailable — ESP32 not connected to this road.
                                            </div>
                                        )}
                                    </div>

                                    {/* ── PIEZO — only show if ESP32 online ── */}
                                    {espUp && (
                                        <div style={{
                                            background: piezoActive ? '#1a1000' : '#0f172a',
                                            border: `1px solid ${piezoActive ? '#f59e0b' : '#1e293b'}`,
                                            ...
                                            <div style={{ fontSize: 10, color: '#64748b', marginTop: 5 }}>
                                                {piezoActive
                                                    ? 'Heavy vehicle detected (IR + vibration confirmed) → +3s Green extension'
                                                    : 'Monitoring for heavy vehicle vibration.'}
                                            </div>
                                            {piezoActive && (
                                                <div style={{ marginTop: 5, fontSize: 10, color: '#fb923c', fontWeight: 'bold' }}>
                                                    ⚡ Green time extended by +3s during next decision
                                    ...
                                    {/* ── GOOGLE TRAFFIC — show for all roads (works even without ESP32) ── */}
                                    <div style={{
                                        background: '#0f172a', borderRadius: 8, padding: 10, marginBottom: 8,
                                        border: `1px solid ${googleWorking ? '#1e3a5f' : '#2d2020'}`
                                    }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                            <span style={{ fontSize: 13 }}>🗺️</span>
                                            <span style={{ fontSize: 11, color: '#64748b' }}>Next Intersection Traffic:</span>
                                            {googleWorking ? (
                                                <>
                                                    <TrafficBadge level={google} />
                                                    <span style={{ fontSize: 10, color: '#64748b' }}>
                                                        {!espUp ? '(used for signal timing)' : scenario === 'IR' ? '(ranking only)' : '(timing + ranking)'}
                                                    </span>
                                                </>
                                            ) : (
                                                <span style={{ fontSize: 11, color: '#f87171', fontWeight: 'bold' }}>
                                                    ❌ Google API Offline
                                                </span>
                                            )}
                                        </div>
                                        {!espUp && !googleWorking && (
                                            <div style={{ marginTop: 6, fontSize: 11, color: '#ef4444', fontWeight: 'bold' }}>
                                                ⚠️ No data source available — using rotation fallback
                                            </div>
                                        )}
                                        {!espUp && googleWorking && (
                                            <div style={{ marginTop: 6, fontSize: 11, color: '#4ade80' }}>
                                                ✓ Google Traffic is the only data source for this road
                                            </div>
                                        )}
                                    </div>
            ...
            <div style={{ background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 20, marginBottom: 22, border: '1px solid #1e3a5f' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12 }}>
                    {[
                        { label: 'System Mode',        value: decision?.mode || '—',                          ok: decision?.mode === 'BOTH' || decision?.mode === 'SENSOR_ONLY' },
                        { label: 'Current Winner',     value: winner ? `${winner} → GREEN` : 'Starting',     ok: !!winner },
                        { label: 'Yellow Time',        value: `${yellowTime}s ${rainDetected ? '(3s+2s rain)' : '(normal)'}`, ok: true },
                        { label: 'RED for Others',     value: redForOthers > 0 ? `${redForOthers}s (dynamic)` : '—',          ok: redForOthers > 0, note: 'Equals winner Green + Yellow' },
                        { label: 'ESP32 Connected',    value: `${Object.values(espOnline).filter(Boolean).length}/4 roads`,   ok: Object.values(espOnline).some(Boolean) },
                        { label: 'Google Traffic',     value: googleWorking ? 'Active' : 'Offline',           ok: googleWorking },
                        { label: 'Rain Sensor',        value: rainDetected ? 'Rain' : 'Clear',                ok: true },
                        { label: 'Winner Scenario',    value: decision?.winnerScenario || '—',                ok: !!decision?.winnerScenario },
                        { label: 'Green Base',         value: '3s base + IR bonus (Light +3s, Heavy +6s)',    ok: true },
                        { label: 'Offline Road Logic', value: 'Google Only → fallback rotation if no data',   ok: true },
                    ].map(m => (
                        <div key={m.label} style={{ background: '#0f172a', borderRadius: 10, padding: 12, border: `1px solid ${m.ok ? '#22c55e33' : '#ef444433'}` }}>
                            <div style={{ fontSize: 10, color: '#64748b', marginBottom: 4, letterSpacing: 1 }}>{m.label}</div>
                            <div style={{ fontSize: 13, fontWeight: 'bold', color: m.ok ? '#4ade80' : '#f87171' }}>{m.value}</div>
                ...
                <div style={{ background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 20, marginBottom: 22, border: '1px solid #1e3a5f' }}>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid #1e3a5f' }}>
                                    {['Rank','Road','Data Source','Distance','IR Queue','Piezo','Rain','Next Traffic','Score','Green Time','LED'].map(h => (
                                        <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#475569', fontSize: 10, letterSpacing: 1, whiteSpace: 'nowrap' }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {decision.priorities.map((p, i) => {
                                    const irForRoad    = irData[p.road] || { queueLevel: 'None' };
                                    const piezoForRoad = piezoData[p.road] || false;
                                    const roadEspUp    = espOnline[p.road] === true;

                                    // Table scenario badge — show ESP32 OFFLINE for downed roads
                                    const tableScenario = !roadEspUp
                                        ? (googleWorking ? 'GOOGLE_ONLY' : 'NO_DATA')
                                        : (p.sensorScenario || 'FALLBACK');
                                    ...
                                    return (
                                        <tr key={p.road} style={{ borderBottom: '1px solid #0f172a', background: i === 0 ? 'rgba(34,197,94,0.05)' : 'transparent' }}>
                                            ...
                                            <td style={{ padding: '8px 10px', fontWeight: 'bold', color: i === 0 ? '#e2e8f0' : '#94a3b8' }}>
                                                {p.road}
                                                {!roadEspUp && <span style={{ marginLeft: 4, fontSize: 9, color: '#f87171' }}>⚡</span>}
                                            </td>
                                            <td style={{ padding: '8px 10px' }}>
                                                <ScenarioBadge scenario={tableScenario} />
                                                {!roadEspUp && (
                                                    <div style={{ fontSize: 9, color: '#f87171', marginTop: 2 }}>ESP32 OFFLINE</div>
                                                )}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: '#94a3b8' }}>
                                                {roadEspUp && p.distance ? `${Math.round(p.distance)}cm` : '—'}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: roadEspUp ? (irForRoad.queueLevel === 'Heavy' ? '#f87171' : irForRoad.queueLevel === 'Light' ? '#fde047' : '#4ade80') : '#475569', fontWeight: 'bold' }}>
                                                {roadEspUp
                                                    ? (irForRoad.queueLevel === 'Heavy' ? '🔴 HEAVY' : irForRoad.queueLevel === 'Light' ? '🟡 LIGHT' : '🟢 NONE')
                                                    : '— N/A'}
                                            </td>
                                            <td style={{ padding: '8px 10px' }}>
                                                {roadEspUp
                                                    ? <span style={{ color: piezoForRoad ? '#fb923c' : '#475569', fontWeight: 'bold', fontSize: 11 }}>{piezoForRoad ? '🚛 YES' : '—'}</span>
                                                    : <span style={{ color: '#475569', fontSize: 11 }}>— N/A</span>}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: rainDetected ? '#60a5fa' : '#4ade80', fontSize: 11 }}>
                                                {rainDetected ? `🌧️ ${yellowTime}s` : `☀️ ${yellowTime}s`}
                                            </td>
                                            <td style={{ padding: '8px 10px' }}>
                                                {googleWorking ? <TrafficBadge level={p.traffic} /> : <span style={{ color: '#475569', fontSize: 11 }}>— Offline</span>}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: p.score > 0 ? '#4ade80' : p.score < 0 ? '#f87171' : '#94a3b8', fontWeight: 'bold' }}>
                                                {p.score <= -9000 ? '⛔ OFFLINE' : typeof p.score === 'number' ? p.score.toFixed(1) : '—'}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: i === 0 ? '#4ade80' : '#94a3b8', fontWeight: i === 0 ? 'bold' : 'normal' }}>
                                                {p.greenTime ? `${Math.round(p.greenTime)}s` : '—'}
                                            </td>
                    ...
                    <div style={{ marginTop: 12, padding: 10, background: '#0f172a', borderRadius: 8, fontSize: 11, color: '#64748b', border: '1px solid #1e3a5f' }}>
                        ...
                        {winner && decision?.greenDuration
                            ...
                            : "Calculated each cycle as winner's green + yellow duration"}
                        &nbsp;|&nbsp;
                        <span style={{ color: '#f87171' }}>⚡ = ESP32 offline (excluded from winning)</span>
                    </div>
                </div>
            )}
            ...
            <div style={{ background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 16, marginTop: 8, border: '1px solid #1e3a5f' }}>
                <h3 style={{ margin: '0 0 12px', color: '#94a3b8', fontSize: 13 }}>📋 System Rules</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 10, fontSize: 11, color: '#64748b' }}>
                    <div>📡 <strong style={{ color: '#60a5fa' }}>ULTRASONIC mode:</strong> ESP32 online, dist ≥ 20cm → uses distance + Google for timing</div>
                    <div>🔦 <strong style={{ color: '#c084fc' }}>IR mode:</strong> ESP32 online, dist &lt; 20cm → uses IR sensors for timing</div>
                    <div>🗺️ <strong style={{ color: '#4ade80' }}>GOOGLE ONLY:</strong> ESP32 offline, Google API working → Google traffic drives timing</div>
                    <div>❌ <strong style={{ color: '#f87171' }}>NO DATA:</strong> Both ESP32 and Google offline → fallback rotation timing</div>
                    <div>🟢 <strong>GREEN:</strong> 3s base + Light +3s=6s, Heavy +6s=9s, Piezo (IR+vibration) +3s stacked</div>
                    <div>🟡 <strong>YELLOW:</strong> 3s dry, +2s rain = 5s total | Sequence: RED→YELLOW→GREEN→YELLOW→RED</div>
                    <div>🔴 <strong>RED (others):</strong> Dynamic = winner GREEN + YELLOW duration each cycle</div>
                    <div>⚡ <strong>ESP32 Offline:</strong> Road excluded from winning — shown as GOOGLE ONLY or NO DATA</div>
                </div>
            </div>
            ...
            <div style={{ background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 20, marginTop: 22, border: '1px solid #1e3a5f' }}>
                ...
                {analyticsTab === 'livecongestion' && (
                    <div>
                        <p style={{ fontSize: 11, color: '#64748b', marginBottom: 14 }}>
                            Current conditions at Nawinna Junction. Green = easy to pass, Red = heavy wait expected.
                        </p>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
                            {ROADS.map(road => {
                                const roadEspUp = espOnline[road] === true;
                                const ir        = irData[road] || {};
                                const gTraffic  = googleTraffic[road] || 'Unknown';
                                const heavy     = heavyVehicleActive[road];
                                ...
                                if (!roadEspUp && !googleWorking) {
                                    congestion = 'No Data'; waitEst = 'No data source';
                                    tip = 'No ESP32 or Google — rotation fallback active'; barColor = '#475569';
                                } else if (!roadEspUp && googleWorking) {
                                    congestion = gTraffic === 'Heavy' ? 'Heavy' : gTraffic === 'Medium' ? 'Moderate' : 'Low';
                                    waitEst = 'Based on Google only';
                                    tip = 'ESP32 offline — Google Traffic driving decisions';
                                    barColor = gTraffic === 'Heavy' ? '#ef4444' : gTraffic === 'Medium' ? '#f59e0b' : '#22c55e';
                                } else if (ir.queueLevel === 'Heavy' || gTraffic === 'Heavy') {
                                    congestion = 'Heavy';
                                    waitEst = `${(greenTime[road] || 9) + yellowTime}–${(greenTime[road] || 9) * 2}s wait`;
                                    tip = 'Expect delays — consider alternate route'; barColor = '#ef4444';
                                } else if (ir.queueLevel === 'Light' || gTraffic === 'Medium') {
                                    congestion = 'Moderate';
                                    waitEst = `${(greenTime[road] || 6)}–${(greenTime[road] || 6) + yellowTime}s wait`;
                                    tip = 'Some traffic — normal wait time'; barColor = '#f59e0b';
                                } else {
                                    congestion = 'Low'; waitEst = 'Short queue';
                                    tip = 'Good to go'; barColor = '#22c55e';
                                }
                                ...
                                return (
                                    <div key={road} style={{ background: '#0f172a', borderRadius: 12, padding: 14, border: `2px solid ${barColor}44` }}>
                                        ...
                                        {!roadEspUp && (
                                            <div style={{ background: '#1f0a0a', color: '#f87171', border: '1px solid #ef444433', borderRadius: 6, padding: '3px 8px', fontSize: 10, marginBottom: 6, fontWeight: 'bold' }}>
                                                ⚡ ESP32 not connected
                                            </div>
                                        )}
                                        {heavy && roadEspUp && (
                                        ...
                                        <div style={{ fontSize: 11, color: '#64748b' }}>{tip}</div>
                                        <div style={{ background: '#1e293b', borderRadius: 4, height: 6, marginTop: 8 }}>
                                            <div style={{ width: congestion === 'Heavy' ? '85%' : congestion === 'Moderate' ? '50%' : congestion === 'No Data' ? '20%' : '15%', background: barColor, height: '100%', borderRadius: 4, transition: 'width 1s' }} />
                                        </div>
                                    </div>
                                );
                            })}
                            ...
                            {analyticsData.roadPerf.length > 0 ? analyticsData.roadPerf.map(r => {
                                ...
                                return (
                                    <div key={r.road} style={{ background: '#0f172a', borderRadius: 12, padding: 14, border: `1px solid ${color}44` }}>
                                        ...
                                        <div style={{ marginTop: 8, fontSize: 11, color: wait > 30 ? '#ef4444' : wait > 15 ? '#f59e0b' : '#4ade80', fontWeight: 'bold' }}>
                                            {wait > 30 ? '⚠️ Long waits' : wait > 15 ? '⚡ Moderate' : '✅ Short waits — good flow'}
                                        </div>
                                    </div>
                                );
                            }) : (
            ...
            <div style={{ textAlign: 'center', marginTop: 28, color: '#1e3a5f', fontSize: 11 }}>
                HYDRA v5.1 — Fixed Offline Road Badges — Nawinna Junction, Kurunegala
            </div>
        </div>
    );
}
