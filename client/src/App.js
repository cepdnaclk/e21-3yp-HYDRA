...
// Heavy Vehicle Badge
const HeavyVehicleBadge = ({ detected }) => {
    ...
    return (
        <div style={{
            ...
            <span>🚛</span> HEAVY VEHICLE — +5s Green Bonus
        </div>
    );
};
...
const WeatherBadge = ({ isRaining, yellowTime }) => {
    ...
    return (
        <div style={{
            ...
            <span>☀️</span> DRY — Yellow: {yellowTime}s
        </div>
    );
};
...
function App() {
    ...
    useEffect(() => {
        ...
        socket.on('fullState', data => {
            ...
            if (data.irData) {
                ...
                ROADS.forEach(road => {
                    ...
                    if (ir.ir1Blocked && ir.ir2Blocked) trafficDensity = 'Heavy';
                    else if (ir.ir1Blocked || ir.ir2Blocked) trafficDensity = 'Light';
                    newIrData[road] = { 
                        ir1Blocked: ir.ir1Blocked || false, 
                        ir2Blocked: ir.ir2Blocked || false, 
                        trafficDensity 
                    };
                });
                setIrData(newIrData);
        ...
        // Piezo/Heavy Vehicle Update
        socket.on('piezoUpdate', ({ road, heavyVehicle }) => {
            setPiezoData(prev => ({ ...prev, [road]: heavyVehicle }));
            if (heavyVehicle) {
                showNotif(`🚛 Heavy vehicle detected on ${road}! +5s Green bonus applied`, 'info');
            }
        });
        ...
        socket.on('rainUpdate', ({ rainDetected: r, yellowTime: y }) => {
            ...
            setYellowTime(r ? 5 : 3);
            if (r) {
                showNotif(`🌧️ Rain detected! Yellow time extended to 5 seconds`, 'info');
            }
        });
        
        // Pedestrian Update
        socket.on('pedestrianUpdate', ({ road, requested, crossing, duration }) => {
            setPedStatus(prev => ({
                ...prev,
                [road]: { requested: requested || false, crossing: crossing || false, duration: duration || 0 }
            }));
            if (requested && !crossing) {
                showNotif(`🚶 Pedestrian waiting at ${road} road!`, 'info');
            }
            if (crossing) {
                showNotif(`🚶 Pedestrian crossing at ${road} road for ${duration}s`, 'success');
            }
        });
    ...
    const anyPedestrianWaiting = Object.values(pedStatus).some(p => p.requested && !p.crossing);
    const anyPedestrianCrossing = Object.values(pedStatus).some(p => p.crossing);
    const anyHeavyVehicle = Object.values(piezoData).some(v => v === true);
        ...
        <div style={{ padding: 20, fontFamily: "'Segoe UI', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: 'white' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
                ...
                <div style={{ marginTop: 10, display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {anyPedestrianWaiting && (
                        <span style={{ background: '#3d2000', color: '#fde047', padding: '3px 12px', borderRadius: 20, fontSize: 12, fontWeight: 'bold' }}>
                            🚶 PEDESTRIAN WAITING
                        </span>
                    )}
                    {anyPedestrianCrossing && (
                        <span style={{ background: '#1e3a5f', color: '#60a5fa', padding: '3px 12px', borderRadius: 20, fontSize: 12, fontWeight: 'bold' }}>
                            🚶 PEDESTRIAN CROSSING
                        </span>
                    )}
                    {anyHeavyVehicle && (
                        <span style={{ background: '#7f1d1d', color: '#f87171', padding: '3px 12px', borderRadius: 20, fontSize: 12, fontWeight: 'bold' }}>
                            🚛 HEAVY VEHICLE
                        </span>
                    )}
                    <span style={{ background: '#1e293b', color: '#94a3b8', padding: '3px 12px', borderRadius: 20, fontSize: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        {anyHeavyVehicle && (
                            <div style={{ color: '#f87171', fontSize: 11, marginTop: 4 }}>
                                🚛 Heavy vehicle detected — +5s Green bonus applied
                            </div>
                        )}
                    </div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div style={{ flex: 1 }}>
                                    <h3 style={{ margin: '0 0 10px', color: '#cbd5e1', fontSize: 14, letterSpacing: 2, textTransform: 'uppercase' }}>
                                        ...
                                        {heavy && <span style={{ color: '#f87171', fontSize: 11, marginLeft: 8 }}>🚛 HEAVY</span>}
                                    </h3>
                                    ...
                                    <div style={{ background: '#0f172a', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                                            <span>🚛</span>
                                            <span style={{ fontSize: 11, color: '#64748b' }}>Heavy Vehicle Detection</span>
                                        </div>
                                        <HeavyVehicleBadge detected={heavy} />
                                        {heavy && (
                                            <div style={{ fontSize: 10, color: '#f87171', marginTop: 4 }}>
                                                ⚡ Heavy vehicle detected → Green +5 seconds
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div style={{ background: '#0f172a', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                                        ...
                                        <span style={{ fontSize: 11, color: rainDetected ? '#60a5fa' : '#4ade80', marginLeft: 8 }}>
            ...
            <div style={{ background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 20, marginBottom: 22, border: '1px solid #1e3a5f' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
                    {[
                        ...
                        { label: 'Piezo/Heavy Vehicle', value: anyHeavyVehicle ? 'Detected' : 'None', ok: true },
                        { label: 'Rain Sensor', value: rainDetected ? 'Rain Detected' : 'Clear', ok: true },
                        { label: 'Yellow Time', value: `${yellowTime}s`, ok: true },
                        { label: 'Pedestrian', value: anyPedestrianWaiting ? 'Waiting' : anyPedestrianCrossing ? 'Crossing' : 'Idle', ok: true },
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid #1e3a5f' }}>
                                    {['Rank','Road','Distance','IR Traffic','Heavy Veh','Pedestrian','Rain','Google','Score','Green Time','LED'].map(h => (
                            ...
                            <tbody>
                                {decision.priorities.map((p, i) => {
                                    ...
                                    const pedForRoad = pedStatus[p.road] || { requested: false, crossing: false };
                                    const heavyForRoad = piezoData[p.road] || false;
                                    return (
                                        <tr key={p.road} style={{ borderBottom: '1px solid #0f172a', background: i === 0 ? 'rgba(34,197,94,0.05)' : 'transparent' }}>
                                            ...
                                            <td style={{ padding: '8px 10px', color: getDensityColor(irForRoad.trafficDensity), fontWeight: 'bold' }}>
                                                {irForRoad.trafficDensity === 'Heavy' ? '🔴 HEAVY' : irForRoad.trafficDensity === 'Light' ? '🟡 LIGHT' : '🟢 NONE'}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: heavyForRoad ? '#f87171' : '#475569' }}>
                                                {heavyForRoad ? '🚛 Yes (+5s)' : 'No'}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: pedForRoad.crossing ? '#60a5fa' : pedForRoad.requested ? '#fde047' : '#475569' }}>
                                                {pedForRoad.crossing ? '🚶 CROSSING' : pedForRoad.requested ? '🚶 WAITING' : '—'}
                                            </td>
                                            <td style={{ padding: '8px 10px', color: rainDetected ? '#60a5fa' : '#4ade80' }}>
                                                {rainDetected ? '🌧️ Rain' : '☀️ Dry'}
                                             </td>
            ...
            <div style={{ background: 'linear-gradient(160deg,#1a2540,#111827)', borderRadius: 16, padding: 16, marginTop: 22, border: '1px solid #1e3a5f' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 12, fontSize: 11, color: '#64748b' }}>
                    ...
                    <div>🟢 <strong>GREEN Light:</strong> 3s base + traffic bonus (Light: +3s, Heavy: +6s) + Heavy Vehicle: +5s</div>
                    <div>🔦 <strong>IR Sensors:</strong> Both blocked = Heavy Traffic (+6s), One blocked = Light Traffic (+3s)</div>
                    <div>🚛 <strong>Piezo Sensor:</strong> Detects heavy vehicles → +5s Green bonus</div>
                    <div>🌧️ <strong>Rain Sensor:</strong> Detects rain → Yellow extends from 3s to 5s</div>
                    <div>🚶 <strong>Pedestrian:</strong> Button press during RED → immediate crossing; during GREEN → waits for cycle; during YELLOW → shows countdown</div>
                </div>
            </div>

            <div style={{ textAlign: 'center', marginTop: 28, color: '#1e3a5f', fontSize: 11 }}>
                HYDRA v4.0 — Complete Sensor Integration — Nawinna Junction, Kurunegala
            </div>
        </div>
    );
}
