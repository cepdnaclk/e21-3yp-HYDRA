    ...
    if(String(topic) == pedCmdTopic){
        ...
        if(!deserializeJson(pd, msg)){
            ...
            if(action == "START_CROSSING"){
                ...
                // Override pedCrossing duration
                const int crossDur = dur;
                // Will be ended by updatePedestrianCrossing or END_CROSSING
            }
                ...
                // Show countdown on 7-segment while PED_RED stays on
                startPedCountdown(dur);
            }
        }
        return;
